...
# spyce/aliases.py
################################################################################
# Alias handler for the spyce library
#
...
# either 553 or 55076.
#
# Upon importing this module, a new function is defined for every spyce function
# that takes a frame or body as input. The new function has the same name as the
# pre-existing spyce function, but with "_alias" inserted immediately after the
# original spyce name (and before any other suffix such as "_vector" or
# "_error").
#
...
#   spyce.use_noaliases()
#
# You can also select the specific version of the spyce function via a function
# attribute. For example, after calling
#   spyce.use_aliases('spkez')
# the function spyce.spkez will support aliases. However, calling
#   spyce.spkez.noalias(...)
...
import spyce
import spyce.alias_support as support

if not hasattr(spyce, 'ALIASES_IMPORTED'):

    # Upon import, define all missing alias versions
    support._define_all_alias_versions()

    # Also add new functions directly to the spyce module
    spyce.use_aliases = support.use_aliases
    spyce.use_noaliases = support.use_noaliases
    spyce.get_body_aliases = support.get_body_aliases
    spyce.get_frame_aliases = support.get_frame_aliases
    spyce.define_body_aliases = support.define_body_aliases
    spyce.define_frame_aliases = support.define_frame_aliases

################################################################################
# Record the fact that this module was imported
################################################################################

spyce.ALIASES_IMPORTED = True
