...
int *my_int_malloc(int count) {
    double *result = (int *) PyMem_Malloc(count * sizeof(int));
    if (!result) {
        PyErr_SetString(PyExc_MemoryError, "Failed to allocate memory");
        return result;
    }
...
%apply (double RETURN_DOUBLE)   {double ducrss_c};

extern double ducrss_c (
        double s1[6],
        double s2[6],
        double sout[6]);
...
%apply (double IN_ARRAY1[ANY])  {double s2[6]};
%apply (double OUT_ARRAY1[ANY]) {double sout[6]};
%apply (double RETURN_DOUBLE)   {double dvcrss_c};

extern double dvcrss_c (
        double s1[6],
        double s2[6],
        double sout[6]);
...
%apply (double **OUT_ARRAY2, int *DIM1, int *DIM2)
...
%rename (fovray_c) fovray_c;

%apply (double  IN_ARRAY1[ANY]) {double raydir[3]};
%apply (int RETURN_VOID) {void fovray_c};

extern void fovray_c ( 
        char *CONST_CHAR,
        double raydir[3],
        char *CONST_CHAR,
        char *CONST_CHAR,
...
%rename (fovtrg) fovtrg_c;

%apply (int RETURN_VOID) {void fovtrg_c};

extern void fovtrg_c ( 
        char *CONST_CHAR,
        char *CONST_CHAR,
        char *CONST_CHAR,
        char *CONST_CHAR,
...
%apply (char *CONST_STRING) {char *fixref};
%apply (double **OUT_ARRAY2, int *DIM1, int *DIM2)
                    {double **lonlat, int *ldim1, int *ldim2};
%apply (double **OUT_ARRAY2, int *DIM1, int *DIM2)
...
%inline %{
    void my_latsrf_c(char *method, char *target, double et, char *fixref,
        ...
        lonlat1 = my_malloc(npts * 2)
        if (!lonlat1) return;

        srfpts1 = my_malloc(npts * 3)
        if (!srfpts1) {
            free(lonlat1);
            return;
        }

        latsrf_c(method, target, et, fixref, npts, *lonlat1, *srfpts1);
...
%apply (int **OUT_ARRAY1, int *DIM1) {int **npts, int *ndim1};
%apply (double **OUT_ARRAY2, int *DIM1, int *DIM2)
                    {double **points, int *pdim1, int *pdim2};
%apply (double **OUT_ARRAY1, int *DIM1) {double **epochs, int *edim1};
%apply (double **OUT_ARRAY2, int *DIM1, int *DIM2)
...
%inline %{
    void my_limbpt_c(char *method, char *target, double et,
        ...
        limbpt_c(method, target, et, fixref, abcorr, xoelox, obsrvr, refvec,
...
%apply (void RETURN_DOUBLE) {double phaseq_c};
...
%apply (void RETURN_DOUBLE) {double pltar_c};
...
%apply (int DIM1, double IN_ARRAY2[ANY][ANY]) {int nv, double vrtces[1][3]};
%apply (int DIM1,    int IN_ARRAY2[ANY][ANY]) {int np, int plates[1][3]};
%apply (void RETURN_DOUBLE) {double pltvol_c};
...
%inline %{
    void my_srfnrm_c(char *method, char *target, double et, char *fixref,
        ...
        edterm_c(method, target, et, fixref, npts, srfpts, result);
