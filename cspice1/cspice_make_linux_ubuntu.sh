...
rm -f cspice.py cspice_swig_wrap.c cspice_swig_wrap.o

swig -python cspice_swig.i

gcc -c `python-config --cflags` cspice_swig_wrap.c -fPIC -I$PYTHON_INCLUDE \
    -I$PYTHON_PKGS/numpy/core/include -Icspice/src/cspice

rm -f _cspice_swig.so

ld -o _cspice_swig.so cspice_swig_wrap.o \
   -shared cspice/lib/cspice.a -lm
